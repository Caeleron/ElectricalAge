    ...
    companion object {
        ...
        fun addRecipe(output: ItemStack?, recipeRows: List<String>, typeKey: Map<Char, ItemStack?>) {
            if (output != null) {
                for (key in typeKey) {
                    if  (key.value == null) {
                        System.out.println("Key value is null in ${key.key}")
                        return
                    }
                    if (!recipeRows.joinToString().contains(key.key)) {
                        System.out.println("Key ${key.key} not in recipie rows: ${recipeRows.joinToString()}")
                        return
                    }
                }
        ...
        fun findItemStack(name: String, stackSize: Int): ItemStack? {
            ...
            if (stack == null) {
                stack = Eln.dictionnaryOreFromMod[name]
                if (stack != null)
