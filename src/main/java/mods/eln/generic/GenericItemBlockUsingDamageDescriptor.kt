...
open class GenericItemBlockUsingDamageDescriptor @JvmOverloads constructor(name: String, iconName: String = name) {
    ...
    fun setDefaultIcon(name: String) {
        ...
        this.iconName = if (Eln.noSymbols && regularIconExists(iconName)) "$iconName-ni" else iconName
        if (Eln.instance.isDevelopmentRun) {
            if (!schematicIconExists(iconName))
                println("There's an icon missing!: assets/eln/textures/blocks/$iconName.png")
            if (!regularIconExists("$iconName-ni"))
                println("There's an icon missing: assets/eln/textures/blocks/$iconName-ni.png")
        }
    }

    private fun schematicIconExists(name: String) = javaClass.classLoader.getResource("assets/eln/textures/blocks/$iconName.png") != null
    private fun regularIconExists(name: String) = javaClass.classLoader.getResource("assets/eln/textures/blocks/$iconName-ni.png") != null
    ...
    fun updateIcons(iconRegister: IIconRegister) {
        icon = iconRegister.registerIcon("eln:$iconName")
    }
